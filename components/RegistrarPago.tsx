...
// ✅ Componente completo `RegistrarPago.tsx`
// Incluye: resumen de pagos + descarga de PDF con formulario completo (descarga automática si es exitoso)
...
const reciboRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (estadiaIdFromUrl) {
        ...
        calcularPendienteYReserva(estadiaIdFromUrl, dataEstadias);
        cargarPagos(estadiaIdFromUrl);
      }
    };
  ...
  const cargarPagos = async (estadiaId: string) => {
  ...
  const calcularPendienteYReserva = (id: string, lista?: any[]) => {
    ...
    const suma = pagos.reduce((acc: number, p: any) => acc + parseFloat(p.monto || 0), 0);
  ...
  const handleTipoPagoChange = (tipoPagoId: string) => {
    ...
    const yaTieneReserva = estadiaSeleccionada?.pagos?.some((p: any) => p.tipo_pago_id === tipoReserva?.id);

    let nuevoMonto = pago.monto;

    if (
      tipoReserva &&
      tipoPagoId === tipoReserva.id &&
      !yaTieneReserva &&
      estadiaSeleccionada?.monto_reserva
    ) {
...
const generarPDF = async () => {
  if (!clienteNombre || !habitacionNumero || !pago.monto) {
    console.warn('Faltan datos para el recibo');
    return;
  }
  ...
  const reciboHTML = `
    ...
    ">
      <h1 style="text-align: center; font-size: 24px; margin-bottom: 1.5rem;">Recibo de Pago</h1>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const tipoSeleccionado = tiposPago.find(tp => tp.id === pago.tipoPagoId);
    const esReserva = tipoSeleccionado?.descripcion?.toLowerCase() === 'reserva';
    ...
    if (res.ok) {
      ...
      calcularPendienteYReserva(pago.estadiaId);
